...
airflow trigger_dag \
    --conf '[curly-braces]"maxLogAgeInDays":30[curly-braces]' airflow-log-cleanup
--conf options:
...
try:
    ...
    logging.error(f"While getting BASE_LOG_FOLDER: {e}")
...
if [ ! -f """
    ...
    if [ "${REMOVE_LOCK_FILE_EXIT_CODE}" != "0" ]; then
        ...
        To re-run the DAG, ensure that the lock file has been deleted (
    """
    + str(LOG_CLEANUP_PROCESS_LOCK_FILE)
    + """)."
        exit ${REMOVE_LOCK_FILE_EXIT_CODE}
